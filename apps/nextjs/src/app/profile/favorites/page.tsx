...
export default function MyFavoritesPage() {
  ...
  const { data: favoritesData, isLoading: favoritesLoading } = api.favorite.myFavorites.useQuery(
    { limit, offset },
  ...
  if (!session?.user) {
    router.push("/auth/signin?callbackUrl=" + encodeURIComponent("/profile/favorites"));
    return null;
  }

  const listings = favoritesData?.listings ?? [];
  const totalCount = favoritesData?.total ?? 0;
  const hasMore = favoritesData?.hasMore ?? false;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-600">
              {t("favoritesCount", { count: totalCount })}
            </p>
          </div>
        </div>
      </div>
